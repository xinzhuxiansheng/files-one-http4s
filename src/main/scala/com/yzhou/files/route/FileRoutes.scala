...
object FileRoutes :
  def fileServiceRoutes[F[_] : Concurrent : Async: Applicative](fileService: FileService[F]): HttpRoutes[F] =
...
//          //filePartOption = multipart.parts.find(p => p.name.contains("file"))
//
//          response <- (filePartOption) match {
//            case (Some(filePart: Part[F])) => // 确保 filePart 是 Part[F] 类型
//              // 提取 path 参数
//              val saveResult = fileService.uploadFile("/a", filePart.filename.getOrElse("unnamed"), filePart.body)
//              Ok(saveResult)
//            case (None) =>
